...
export default function EditPurchaseScreen({ route, navigation }) {
    ...
    useEffect(() => {
        var client = new W3CWebSocket(`ws://localhost:4567/getDiscountAndPurchasesPolicies`);
        ...
        client.onopen = function () {
            if (basedOn == 'product') {
                client.send(JSON.stringify({ "type": "GET_PURCHASE_POLICY_PRODUCT", "userId": userId, "storeId": storeId, "productId": route.params.productId }));
            } else if (basedOn == 'category') {
                client.send(JSON.stringify({ "type": "GET_PURCHASE_POLICY_CATEGORY", "userId": userId, "storeId": storeId, "category": route.params.category }));
            } else if (basedOn == 'store') {
                client.send(JSON.stringify({ "type": "GET_PURCHASE_POLICY_STORE", "userId": userId, "storeId": storeId }));
            }
        };
