...
public class PurchasesOnProductsWrapper {

    private Map<Product, ImmediatePurchase> value;

    public PurchasesOnProductsWrapper() {
        this.value = new ConcurrentHashMap<>();
    }

    public void add(int storeId, Product product, ImmediatePurchase immediatePurchase) {
        try {
           ...
           // if(discount instanceof ConditionalDiscount) {
            ...
            Dao<StorePoliciesOnProductsDAO, String> storePoliciesOnProductsDAO = DaoManager.createDao(connectionSource, StorePoliciesOnProductsDAO.class);
            StorePoliciesOnProductsDAO storePoliciesOnProdDaoObj = new StorePoliciesOnProductsDAO(storeId, product.getId(), immediatePurchase.getId());
            storePoliciesOnProductsDAO.create(storePoliciesOnProdDaoObj);
    ...
    public ImmediatePurchase get(Product product) {
        ImmediatePurchase ip = this.value.get(product);
        if (ip != null) return ip;
        else {
            try {
                ConnectionSource connectionSource = connect();
                Dao<StorePoliciesOnProductsDAO, String> storePoliciesOnProductsDAOManager = DaoManager.createDao(connectionSource, StorePoliciesOnProductsDAO.class);
                Map<String, Object> fields = new HashMap<>();
                fields.put("storeId", product.getStoreId());
                fields.put("productId", product.getId());
                List<StorePoliciesOnProductsDAO> storePolOnProdDAOs = storePoliciesOnProductsDAOManager.queryForFieldValues(fields);
                StorePoliciesOnProductsDAO value = storePolOnProdDAOs.get(0);
