    ...
    public void add(Product product, Discount discount) {
        ...
        try {
            ...
            if(discount instanceof ConditionalDiscount) {
                ...
                for (Policy pol : policies) {
                    if(pol instanceof DiscountByMinimalAmount) {
                     Dao<DiscountByMinimalAmountDAO, String> discountByMinimalAmountDAO = DaoManager.createDao(connectionSource, DiscountByMinimalAmountDAO.class);
                        DiscountByMinimalAmountDAO minAmountDaoObj = new DiscountByMinimalAmountDAO(discount.getId(),
                        ...
                        DiscountByMinimalCostDAO minCostDaoObj = new DiscountByMinimalCostDAO(discount.getId(),
                                Integer.parseInt(pol.getPolicyParams().get(0)));
                        ...
                        DiscountByPurchaseTimeDAO purchaseTimeDaoObj = new DiscountByPurchaseTimeDAO (discount.getId(),
                ...
                ConditionalDiscountDAO condDisDaoObj = new ConditionalDiscountDAO(discount.getId(), tmp.getId());
