...
export default function GlobalPurchases({ route, navigation }) {
  const { userId, registered } = route.params;
  const [purchases, setPurchases] = useState([]);
  useEffect(() => {
    var client = new W3CWebSocket('ws://localhost:4567/AdminWebSocket');
    client.onerror = function () {
      console.log('Connection Error');
    };

    client.onopen = function (event) {
      client.send(JSON.stringify({ "type": "GET_GLOBAL_PURCHASES", "userId": userId }));
    }
    ...
    client.onmessage = function (e) {
      ...
      if (parsedMessage.type == "GET_GLOBAL_PURCHASES") {
